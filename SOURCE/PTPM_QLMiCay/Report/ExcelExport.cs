...
namespace Report
{
    public class ExcelExport
    {
        ...
        public const string T_DanhMucKhoa = "PhieuDat";
        #endregion
        ...
        private const string V_NHACUNGCAP = "%NhaCungCap";
        private const string V_TONGTIEN = "%TongTien";
        private const string V_NGUOILAP = "%NguoiLap";
        private const string V_PHIEUDATID = "%PhieuDatId";
        ...
        private MemoryStream GetTemplateStream(string viewName)
        {
         ...
         //   string path = Global.AppPath + Constants.FOLDER_TEMPLATES + Constants.CHAR_FLASH; //+ Constants.FOLDER_EXCELS + Constants.CHAR_FLASH;
            ...
            switch (viewName)
            {
                ...
                case T_DanhMucKhoa:
                    // path += Constants.FILE_KEHOACHTD;
                    arrByte = File.ReadAllBytes("PhieuDat.xls").ToArray();
                    break;
                #endregion
            }
            // Get stream
        ...
        public bool ExportPhieuDat(List<CTPhieuDatDTO> dataSource, PhieuDatDTO phDat, string Ncc,ref string fileName, bool isPrintPreview)
        {
            // Check if data is null
            ...
            for (int i = 1; i <= dataSource.Count; i++)
            {
               dataSource[i - 1].STT = i.ToString();
                dataSource[i - 1].Nl_thanhTien = dataSource[i - 1].Nl_sl * dataSource[i - 1].Nl_donGia;
                sum += dataSource[i - 1].Nl_thanhTien;
            }
          //  string tongSo = dataSource.Sum(t => t.SoLuong).Value.ToString();
            // Create replacer
            Dictionary<string, string> replacer = new Dictionary<string, string>();

            replacer.Add(V_NHACUNGCAP, Ncc);
            replacer.Add(V_TONGTIEN, sum + "");
            replacer.Add(V_NGUOILAP, phDat.SentSupplierBy);
            replacer.Add(V_PHIEUDATID, phDat.PD_Id + "");



            BuildReplacerCurrentDate(ref replacer);

            return OutSimpleReport(dataSource, replacer, "PhieuDat", isPrintPreview, ref fileName);
        }
